...
> Note: `@types` is reserved by TypeScript for ambient declarations. Always use `@site-types` when importing from `src/types/`.
...
- Import from `@types/site-config.types` — use `@site-types/site-config.types` instead
